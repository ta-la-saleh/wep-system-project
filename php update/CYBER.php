...
$result = mysqli_query($conn, "SELECT * FROM course WHERE major_id = 3 ORDER BY id DESC");
?>
 <!--  cyber صفحة تخصص -->
<!DOCTYPE html>
<html >
<head >
    ...
    <meta charset="UTF-8">
    <title>Cybersecurity Major </title>
...
<section class="about-section">
    ...
    <h4>
       <b> Cybersecurity :</b> focuses on protecting systems, networks, and data from digital threats
        by detecting attacks and preventing breaches.
    </h4>
</section>
<main>
   ...
   // نتحقق من وجود كورسات في قاعدة البيانات
        if(mysqli_num_rows($result) > 0) {
            while($row = mysqli_fetch_array($result)) { 
                ...
                $course_image = "photo/Cyber-Security2.jpg"; // الصوره  (أساسيات الأمن السيبراني)
                 $course_id = $row['id'];
            ...
            "SELECT * FROM favorites
             ...
             // تغيير الصورة حسب عنوان الكورس
                if (strpos($course_title, 'هجوم') !== false || strpos($course_title, 'اختراق') !== false || strpos($course_title, 'attack') !== false) {
                    $course_image = "photo/Cyber-Security3.jpg"; // صورة كورس تقنيات الهجوم والاختراق
                } elseif (strpos($course_title, 'مسار') !== false || strpos($course_title, 'عمل') !== false || strpos($course_title, 'وظيف') !== false) {
                    $course_image = "photo/Cyber-Security-4.jpg"; // صورة كورس المسار الوظيفي
                }
        ?>
                        <!-- بطاقة عرض الكورس -->
...
<div class="card-action">
    ...
    <a href="offer.php?id=<?php echo $row['id']; ?>" class="view-btn">
        ...
        <?php 
            ...
            // تطلع اذا ما تم إدخال كورسات  في الأدمن 
